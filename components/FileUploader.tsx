...
const FileUploader = ({ ownerId, accountId, className }: Props) => {
  ...
  //keeping the track of files
  const [files, setFiles] = useState<File[]>([]);
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      // Do something with the files
      setFiles(acceptedFiles);
      //if we are uploading multiple files we need upload them one by one and
      //handle that the file accepted according to our requirement
      const uploadPromises = acceptedFiles.map(async (file) => {
        //clearing the files which donot match the max file criteria
        if (file.size > MAX_FILE_SIZE) {
          setFiles((prevFiles) => prevFiles.filter((f) => f.name !== file.name));
          //if file is to large we cannot store the file thus return error toast
          return toast({
            description: (
  ...
  //this removes the file while we are uploading the file and provdides option to cancel upload
